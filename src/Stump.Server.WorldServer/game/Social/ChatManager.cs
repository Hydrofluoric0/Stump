...
namespace Stump.Server.WorldServer.Game.Social
{
    public class ChatManager : DataManager<ChatManager>, ISaveable
    {
        ...
        /// </summary>
        [Variable]
        public static readonly RoleEnum AdministratorChatMinAccess = RoleEnum.GameMaster;
        ...
        public bool CanUseChannel(Character character, ChatActivableChannelsEnum channel)
        {
            switch (channel)
            {
                ...
                case ChatActivableChannelsEnum.PSEUDO_CHANNEL_FIGHT_LOG:
                    return false;
                case ChatActivableChannelsEnum.CHANNEL_MONDE:
                    return true;
                case ChatActivableChannelsEnum.CHANNEL_VIP:

                    if (character.WorldAccount.Vip == true)
                        return true;
        ...
        public void SayMonde(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            if (!CanUseChannel(client.Character, ChatActivableChannelsEnum.CHANNEL_MONDE))
                return;

            World.Instance.ForEachCharacter(entry =>
            {
                if (objectItems != null)
                    SendChatServerWithObjectMessage(entry.Client, client.Character, ChatActivableChannelsEnum.CHANNEL_MONDE, msg, objectItems);
                else
                    SendChatServerMessage(entry.Client, client.Character, ChatActivableChannelsEnum.CHANNEL_MONDE, msg);
            });
        }

        public void SayVip(WorldClient client, string msg, IEnumerable<ObjectItem> objectItems)
        {
            if (!CanUseChannel(client.Character, ChatActivableChannelsEnum.CHANNEL_VIP))
                return;
