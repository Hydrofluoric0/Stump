...
namespace Stump.Server.WorldServer.Game
{
    public class World : DataManager<World>
    {
        ...
        {
            var Joueurco = World.Instance.GetCharacters().ToList(); // Le serveur v�rifie les personnages qui sont connect� dans la Base de donn�e
            foreach (var character in Joueurco)
            {
                character.SendSystemMessage(3, true); // Le serveur envoie  � tout les joueurs connect� un message de red�marrage serveur

            }
        }
        public void SendAnnounceAllPlayersEvent() // Nom au hasard (Mettre un nom reconnaissable quand m�me...)
        {
            var connectedcharacter = World.Instance.GetCharacters().ToList(); // Le serveur v�rifie les personnages qui sont connect� dans la Base de donn�e
            foreach (var character in connectedcharacter)
            {
                character.DisplayNotification("<br>UN EVENT VA COMMENCER</br> Pr�pares toi � gagner ! "); // Affichage temporaire (Notification)
