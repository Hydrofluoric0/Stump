        ...
        public override void Execute(TriggerBase trigger)
        {
            ...
            foreach (var target in GetTargets(trigger))
            {
                ...
                IPCAccessor.Instance.SendRequest(message,
                    ok => trigger.Reply("Account {0} banned for {1} minutes. Reason : {2}", target.Account.Login, trigger.Get<int>("time"), reason),
                    error => trigger.ReplyError("Account {0} not banned : {1}", target.Account.Login, error.Message));

                World.Instance.SendAnnounce("★ Le joueur <b>" + target.Client.Character.Name + "</b>, après avoir bu quelques verres à la taverne d'Astrub, s'est égaré du droit chemin. Il a donc été banni de ce monde pour : " + reason + ".", System.Drawing.Color.Red);
