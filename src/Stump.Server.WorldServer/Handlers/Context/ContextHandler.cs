...
namespace Stump.Server.WorldServer.Handlers.Context
{
    public partial class ContextHandler : WorldHandlerContainer
    {
        ...
        public static void HandleEnterHavenBagRequestMessage(WorldClient client, EnterHavenBagRequestMessage message)
        {
            ...
            Cell cell;
            var map = World.Instance.GetMap(162793472);
            var previousmapid = player.Record.MapId;
            var previouscellid = player.Record.CellId;
            var previousMap = World.Instance.GetMap(previousmapid);
            cell = map.GetCell(previouscellid);
            ...
            {
                player.SendServerMessage("Tu ne peut pas utiliser le Havre sac en donjon.", Color.Red);
                return;
            }
            else if (player.IsInFight())
            {
                player.SendServerMessage("Tu ne peut pas utiliser le Havre sac  en combat.", Color.Red);
                return;
            }

            if (player.Map == map)
            {
                player.Teleport(previousMap, cell);
            }
            else
            {
                cell = map.GetCell(257);

                if (!player.HasEmote(EmotesEnum.SHADRAGON_EMOTE))
                    player.AddEmote(EmotesEnum.SHADRAGON_EMOTE);

                player.PlayEmote(EmotesEnum.SHADRAGON_EMOTE);
                Thread.Sleep(3200);
